...
#check{
  ...
  <body>
    ...
    <header>
      ...
      <div class="left_area">
      <h3>Cuisine <span>Manel</span></h3>      </div>
            ...
            <div class="col-md-4 col-md-offset-4" style="margin-top:70px;">
                ...
                <form action="{{Route('modifier')}}" method="post" enctype="multipart/form-data">
                    ...
                    <div class="form-group">
                        ...
                        <input type="text" class="form-control" placeholder="Entrer le nom du recette" name="name_recette" value="{{$recette->name_recette}}">
                    ...
                    <select class="form-control" name="difficulté">
                        <option value='{{$recette->difficulté}}' selected>{{$recette->difficulté}}</option>
                    ...
                    <select class="form-control" name="categorie">
                        <option value='{{$recette->categorie}}' selected>{{$recette->categorie}}</option>
                    ...
                    <div class="form-group">
                        ...
                        <input type="number" class="form-control" placeholder="Entrer le durée du préparation" name="preparation" value="{{$recette->preparation}}">
                        <span class="text-danger" >@error('preparation'){{$message}} @enderror</span>
                    </div>
                    <div class="form-group">
                        <label for="cuisson">Cuisson</label>
                        <input type="number" class="form-control" placeholder="Entrer le durée du cuisson" name="cuisson" value="{{$recette->cuisson}}">
                        <span class="text-danger" >@error('cuisson'){{$message}} @enderror</span>
                    </div>
                    <div class="form-group">
                        <label for="ingrédiant">Ingrédiant</label>
                        <textarea class="form-control" placeholder="Entrer les ingrédiants" name="ingrédiant" >{{$recette->ingrédiant}}</textarea>
                        <span class="text-danger" >@error('ingrédiant'){{$message}} @enderror</span>
                    </div>
                    <div class="form-group">
                        <label for="description">Comment le préparer</label>
                        <textarea class="form-control" type="text" placeholder="Comment préparer cette recette" name="description">{{$recette->description}}</textarea>
                        <span class="text-danger" >@error('description'){{$message}} @enderror</span>
                    </div>
                    <div class="form-group">
                        <label for="photo_recette">Photo du recette</label>
                        <input type="file" class="form-control" name="photo_recette">
                        <span class="text-danger" >@error('photo_recette'){{$message}} @enderror</span>
                    </div>
                    <div class="form-group">
                        <input type="hidden" class="form-control" name="id" value="{{$recette->id}}">
                    </div>
                    <div class="form-group">
                        <button class="btn btn-block btn-primary" type="submit">Modifier recette</button>
                    </div>
                </form>
            </div>  
        </div>
